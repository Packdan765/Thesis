...
class FreeLLMHandler:
    ...
    def _initialize_groq(self):
        ...
        try:
            ...
            # If not in environment, try to read from groq_key.txt
            if not api_key:
                key_file = Path(__file__).parent.parent.parent / "groq_key.txt"
                if key_file.exists():
                    try:
                        api_key = key_file.read_text().strip()
                        print(f"[OK] Loaded Groq API key from groq_key.txt")
                        # Set it in environment for future use
                        os.environ["GROQ_API_KEY"] = api_key
                    except Exception as e:
                        print(f"[WARN] Failed to read groq_key.txt: {e}")
                        api_key = None
