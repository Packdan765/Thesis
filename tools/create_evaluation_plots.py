...
class HRLEvaluationPlotter:
    ...
    def __init__(self, experiment_dir):
        ...
        # Detect hypothesis from directory name or metadata
        ...
        if metadata_file.exists():
            with open(metadata_file, 'r') as f:
                ...
                # Extract hypothesis from experiment name (h1, h2, h3, h4, h5, h6)
                for h in ['h1', 'h2', 'h3', 'h4', 'h5', 'h6']:
                    if h in exp_name:
                        self.hypothesis = h.upper()
                        break
        
        # Fallback: check directory name
        if not self.hypothesis:
            dir_name = self.exp_dir.name.lower()
            for h in ['h1', 'h2', 'h3', 'h4', 'h5', 'h6']:
                if dir_name.startswith(h) or f'_{h}_' in dir_name:
                    self.hypothesis = h.upper()
                    break
        
        # Default to H1 if not found (for backwards compatibility)
        if not self.hypothesis:
            self.hypothesis = 'H1'
        
        # Detect if this is flat RL
        self.is_flat_rl = False
        if metadata_file.exists():
            with open(metadata_file, 'r') as f:
                metadata = json.load(f)
                # Primary check: mode field
                if metadata.get('mode', '') == 'flat':
                    self.is_flat_rl = True
                # Fallback: experiment name
                elif 'flat' in metadata.get('experiment_name', '').lower() or 'h1' in metadata.get('experiment_name', '').lower():
                    self.is_flat_rl = True
        
        # Also check directory name as final fallback
        ...
        if self.is_flat_rl:
            print(f"[+] Detected: Flat RL (will use actions instead of options)")
    ...
    def plot_option_usage_distribution(self):
        """RQ1: Strategy evolution - how does option/action usage change over training?"""
        if self.is_flat_rl:
            # For flat RL, use flat action counts
            flat_action_counts = self.metrics.get('flat_action_counts', {})
            if not flat_action_counts:
                # Try to reconstruct from episode data
                episode_option_usage = self.metrics.get('episode_option_usage', [])
                if episode_option_usage:
                    # In flat RL, option_usage might contain flat action names
                    all_actions = set()
                    for ep_usage in episode_option_usage:
                        all_actions.update(ep_usage.keys())
                    # Check if these look like flat actions (contain underscore)
                    if any('_' in str(a) for a in all_actions):
                        # These are flat actions
                        episode_option_usage = episode_option_usage  # Use as-is
                    else:
                        print("   [!] No flat action usage data found")
                        return
                else:
                    print("   [!] No per-episode action usage data")
                    return
            else:
                # Convert flat_action_counts to episode format for plotting
                # Create a single episode with all actions
                episode_option_usage = [flat_action_counts]
                all_actions = set(flat_action_counts.keys())
        else:
            # Hierarchical RL: use options
            episode_option_usage = self.metrics.get('episode_option_usage', [])
        ...
        if not all_actions:
            print("   [!] No option/action data found")
            return
        
        # Calculate percentages per episode
        episodes = np.arange(1, len(episode_option_usage) + 1)
        action_percentages = {act: [] for act in all_actions}
        
        for ep_usage in episode_option_usage:
            total = sum(ep_usage.values())
            if total == 0:
                for act in all_actions:
                    action_percentages[act].append(0)
            else:
                for act in all_actions:
                    pct = (ep_usage.get(act, 0) / total) * 100
                    action_percentages[act].append(pct)
        
        n_episodes = len(episode_option_usage)
        
        # Use binning/averaging for cleaner visualization
        # Target: max 50 points for very long runs, fewer for shorter runs
        max_points = min(50, max(20, n_episodes // 20))
        n_bins = max_points
        
        if n_episodes > 50:
            # Bin episodes and average within each bin
            bin_edges = np.linspace(0, n_episodes, n_bins + 1, dtype=int)
            bin_centers = []
            action_percentages_binned = {act: [] for act in all_actions}
            ...
            for i in range(len(bin_edges) - 1):
                ...
                if start_idx < end_idx:
                    # Average episodes in this bin
                    bin_center = (start_idx + end_idx - 1) / 2 + 1  # Center of bin (1-indexed)
                    bin_centers.append(bin_center)
                    
                    for act in all_actions:
                        bin_data = np.array(action_percentages[act][start_idx:end_idx])
                        bin_avg = np.mean(bin_data) if len(bin_data) > 0 else 0.0
                        action_percentages_binned[act].append(bin_avg)
            
            episodes_plot = np.array(bin_centers)
            action_percentages_plot = action_percentages_binned
        else:
            # For short runs, apply smoothing but plot all points
            window_size = max(3, n_episodes // 10)
            episodes_plot = episodes
            action_percentages_plot = {}
            for act in all_actions:
                data = np.array(action_percentages[act])
                if len(data) >= window_size:
                    smoothed = np.convolve(data, np.ones(window_size)/window_size, mode='same')
                    action_percentages_plot[act] = smoothed
                else:
                    action_percentages_plot[act] = data
        
        # Create plot
        fig, ax = plt.subplots(figsize=(12, 6))
        
        colors = sns.color_palette("Set2", len(all_actions))
        for i, act in enumerate(all_actions):
            # Plot with no markers, just smooth lines
            ax.plot(episodes_plot, action_percentages_plot[act], 
                   label=act, color=colors[i], linewidth=2.5, alpha=0.8)
        
        ax.set_xlabel('Episode', fontsize=12)
        ax.set_ylabel('Usage Frequency (%)', fontsize=12)
        title = f'{self.hypothesis}: Action Usage Evolution Over Training' if self.is_flat_rl else f'{self.hypothesis}: Option Usage Evolution Over Training'
        ax.set_title(title, fontweight='bold', pad=15, fontsize=14)
        ax.legend(loc='best', fontsize=10, framealpha=0.9)
        ax.grid(True, alpha=0.3)
        ax.set_ylim(-5, 105)
        
        # Add reference line (equal distribution)
        ...
        plt.tight_layout()
        filename = 'action_distribution.png' if self.is_flat_rl else 'option_distribution.png'
        save_path = self.dirs['hierarchy'] / filename
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()
        plot_name = 'Action distribution' if self.is_flat_rl else 'Option distribution'
        print(f"   [+] {plot_name} -> {save_path.name}")
    ...
    def plot_option_persistence(self):
        ...
        ax.set_title(f'{self.hypothesis}: Option Persistence (Multi-Turn Coherence)', fontweight='bold', pad=15)
    ...
    def plot_option_evolution(self):
        ...
        ax.set_title(f'{self.hypothesis}: Option Usage Evolution During Training', fontweight='bold', pad=15)
        ax.legend(loc='best')
        ax.grid(True, alpha=0.3)
        ax.set_xticks(x)
    ...
    def plot_subaction_diversity(self):
        """H6: Subaction diversity (entropy) over training for option granularity analysis"""
        episode_subaction_usage = self.metrics.get('episode_subaction_usage', [])
        
        if not episode_subaction_usage:
            print("   [!] No subaction usage data for diversity analysis")
            return
        
        import math
        
        # Compute per-episode subaction entropy
        diversities = []
        for ep_data in episode_subaction_usage:
            total = sum(ep_data.values())
            if total == 0:
                diversities.append(0.0)
                continue
            probs = [count / total for count in ep_data.values()]
            ent = -sum(p * math.log(p + 1e-10) for p in probs if p > 0)
            diversities.append(ent)
        
        if not diversities:
            print("   [!] Could not compute subaction diversity")
            return
        
        episodes = np.arange(1, len(diversities) + 1)
        
        fig, ax = plt.subplots(figsize=(12, 6))
        
        ax.plot(episodes, diversities, alpha=0.4, color='teal', linewidth=0.8)
        
        if len(diversities) >= 10:
            window = min(50, max(5, len(diversities) // 10))
            moving_avg = np.convolve(diversities, np.ones(window)/window, mode='valid')
            ax.plot(range(window, len(diversities) + 1), moving_avg,
                   linewidth=2.5, color='darkcyan', label=f'{window}-episode MA')
        
        ax.axhline(y=np.mean(diversities), color='gray', linestyle=':', alpha=0.5,
                  label=f'Mean: {np.mean(diversities):.3f}')
        
        ax.set_xlabel('Episode')
        ax.set_ylabel('Subaction Entropy (bits)')
        ax.set_title('H6: Subaction Diversity Over Training\n(Higher = More Behavioral Flexibility)', 
                    fontweight='bold', pad=15)
        ax.legend(loc='best')
        ax.grid(True, alpha=0.3)
        
        plt.tight_layout()
        save_path = self.dirs['hierarchy'] / 'subaction_diversity.png'
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"   [+] Subaction diversity -> {save_path.name}")
    
    def plot_option_entropy_curve(self):
        """H6: Option entropy (collapse resistance) over training"""
        episode_option_usage = self.metrics.get('episode_option_usage', [])
        
        if not episode_option_usage:
            print("   [!] No option usage data for entropy analysis")
            return
        ...
        for i in range(len(episode_option_usage)):
            start = max(0, i - window + 1)
            window_data = episode_option_usage[start:i+1]
            
            combined_counts = defaultdict(int)
            for ep_data in window_data:
                for option, count in ep_data.items():
            ...
            probs = [count / total for count in combined_counts.values()]
            ent = -sum(p * math.log(p + 1e-10) for p in probs if p > 0)
            entropies.append(ent)
        
        if not entropies:
            print("   [!] Could not compute option entropy")
            return
        ...
        ax.plot(episodes, entropies, linewidth=2, color='coral', label=f'{window}-episode rolling entropy')
        
        ax.axhline(y=np.mean(entropies), color='gray', linestyle=':', alpha=0.5,
                  label=f'Mean: {np.mean(entropies):.3f}')
        
        ax.set_xlabel('Episode')
        ax.set_ylabel('Option Entropy (bits)')
        ax.set_title('H6: Option Entropy Over Training\n(Higher = Less Option Collapse)', 
                    fontweight='bold', pad=15)
        ax.legend(loc='best')
        ax.grid(True, alpha=0.3)
        
        plt.tight_layout()
        save_path = self.dirs['hierarchy'] / 'option_entropy.png'
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"   [+] Option entropy -> {save_path.name}")
    ...
    def plot_dwell_over_training(self):
        ...
        ax.set_title(f'{self.hypothesis}: Engagement (Dwell) Over Training', fontweight='bold', pad=15)
        ax.legend(loc='best')
        ax.grid(True, alpha=0.3)
    ...
    def plot_exhibit_coverage(self):
        ...
        ax.set_title(f'{self.hypothesis}: Museum Coverage - Exploring Diverse Content', fontweight='bold', pad=15)
        ax.legend(loc='best')
        ax.grid(True, alpha=0.3)
    ...
    def plot_facts_presented(self):
        ...
        ax.set_title(f'{self.hypothesis}: Content Novelty - Facts Presented Per Episode', fontweight='bold', pad=15)
        ax.legend(loc='best')
        ax.grid(True, alpha=0.3)
    ...
    def plot_exhibit_coverage_heatmap(self):
        ...
        ax.set_title(f'{self.hypothesis}: Coverage & Facts Evolution (bubble size = facts presented)', 
    ...
    def generate_all_plots(self):
        ...
        print("\n" + "="*80)
        if self.is_flat_rl:
            print("GENERATING FLAT RL EVALUATION PLOTS")
        else:
        ...
        if not self.is_flat_rl:
            # Skip hierarchical plots for flat RL
            print("\n[2] Hierarchical Control (RQ1):")
            self.plot_option_usage_distribution()
            self.plot_option_persistence()
            self.plot_option_evolution()
            # H6-specific plots (option granularity analysis)
            self.plot_subaction_diversity()
            self.plot_option_entropy_curve()
        else:
            print("\n[2] Action Distribution (Flat RL):")
            # For flat RL, show flat action distribution
            self.plot_option_usage_distribution()  # This already handles flat RL
            print("   [!] Hierarchical-only plots (persistence, evolution) skipped for flat RL")
    ...
    def generate_summary(self):
        ...
        with open(summary_path, 'w', encoding='utf-8') as f:
            ...
            summary_json_path = self.exp_dir / 'summary.json'
            if summary_json_path.exists():
                try:
                    with open(summary_json_path, 'r') as sf:
                        summary_data = json.load(sf)
                        if 'training_duration_formatted' in summary_data:
                            f.write(f"Training Duration: {summary_data['training_duration_formatted']}\n")
                            if 'training_duration_hours' in summary_data:
                                f.write(f"Training Duration (hours): {summary_data['training_duration_hours']:.2f}\n")
                        f.write("\n")
                except Exception:
                    pass  # If summary.json doesn't exist or is malformed, skip
            ...
            reward_avg = summary.get('reward_breakdown_avg', {})
            if reward_avg:
                f.write("REWARD COMPONENTS (Average per Episode):\n")
                total_avg = sum(reward_avg.values())
                for component, avg_value in sorted(reward_avg.items(), key=lambda x: -abs(x[1])):
                    pct = (avg_value / total_avg * 100) if total_avg != 0 else 0.0
                    f.write(f"  {component.capitalize()}: {avg_value:.3f} ({pct:+.1f}% of total)\n")
                f.write(f"\n  Total Average Reward/Episode: {total_avg:.3f}\n\n")
...
def main():
    ...
    parser.add_argument('exp_number', type=int, help='Experiment number (e.g., 7 for exp_007 or major_007)')
    parser.add_argument('--experiment-type', type=str, default=None,
                       choices=['major', 'minor'],
                       help='Experiment type: major or minor (default: search both)')
    args = parser.parse_args()
    
    # Find experiment directory
    exp_base = Path('training_logs/experiments')
    
    # Search for experiments (check all date folders)
    exp_folders = []
    for date_folder in sorted(exp_base.glob('*')):
        if date_folder.is_dir() and date_folder.name.isdigit():
            if args.experiment_type == 'major':
                # Search only for major experiments
                matches = list(date_folder.glob(f'major_{args.exp_number:03d}_*'))
            elif args.experiment_type == 'minor':
                # Search only for minor experiments
                matches = list(date_folder.glob(f'exp_{args.exp_number:03d}_*'))
            else:
                # Search both types
                matches = list(date_folder.glob(f'exp_{args.exp_number:03d}_*')) + \
                         list(date_folder.glob(f'major_{args.exp_number:03d}_*'))
            exp_folders.extend(matches)
    
    if not exp_folders:
        exp_type_str = f" ({args.experiment_type})" if args.experiment_type else ""
        print(f"[ERROR] No experiment found for number {args.exp_number}{exp_type_str}")
        return
    
    # Sort by date and timestamp (most recent first)
    exp_folders = sorted(exp_folders, key=lambda x: x.name, reverse=True)
    target_exp = exp_folders[0]
    
    if len(exp_folders) > 1:
        print(f"[WARNING] Multiple experiments found, using most recent: {target_exp.name}")
        print(f"  Other matches: {[f.name for f in exp_folders[1:]]}")
