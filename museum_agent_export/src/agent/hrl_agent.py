"""
Hierarchical Actor-Critic Agent (Option-Critic Architecture)

This implements the agent that uses Actor-Critic learning for hierarchical
dialogue management using the Options Framework (SMDP).
"""
...
class ActorCriticAgent:
    """
    Actor-Critic Agent using Option-Critic architecture for SMDP models.
    ...
    def select_action(
        ...
        if self.current_option is None:
            option_idx = self._select_option(outputs, available_options, deterministic)
            ...
            should_terminate = (
                (deterministic and term_prob > 0.5) or
                (not deterministic and np.random.random() < term_prob)
            )
